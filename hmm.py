...
class BrownCorpus:
    ...
    def get_q(self, penult_tag, last_tag, current_tag):
        if (penult_tag, last_tag, current_tag) in self.trigram_tag_dict:
            return float(self.trigram_tag_dict[penult_tag, last_tag, current_tag]) / self.bigram_tag_dict[
    ...
    def get_tag_sequence(self, sentence):
        ...
        prob, y[n - 1], y[n] = max(
            [(pi[n, u, v] * self.get_q(u, v, 'STOP'), u, v) for u in self.distinct_tags for v in self.distinct_tags])
    ...
    def test_tag_sequence(self, testFileName, outFileName):
        ...
        fout.close()
        print('elapsed time tag sequence:', finish_time - start_time)
